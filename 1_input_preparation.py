...
if task == 'imputation_between_2_modal':
    ...
    inputdata2 = np.asarray(inputdata2)
    covariates = np.hstack((np.zeros(inputdata1.shape[0]),np.ones(inputdata2.shape[0])))
    full_inputdata2 = np.zeros((inputdata2.shape[0],inputdata1.shape[1]))
    count = 0
    ...
    np.save('results/imputation_input_covariates.npy',to_categorical(covariates, 2))
...
if task == 'integration_paired_data':
    ...
    covariates = np.zeros(inputdata1.shape[0]+inputdata2.shape[0])
    ...
    input_covariates = np.hstack((covariates,covariates,covariates,covariates))
...
if task == 'integration_paired_data_with_batch_effect':
    ...
    covariates = np.hstack((np.zeros(inputdata1.shape[0]),np.ones(inputdata3.shape[0])))
    ...
    sparse.save_npz('results/integration_input_mask.npz', sparse.csr_matrix(input_mask_data))
    sparse.save_npz('results/integration_output_mask.npz', sparse.csr_matrix(output_mask_data))
    np.save('results/integration_input_covariates.npy', to_categorical(input_covariates, 2))
    np.save('results/integration_input_cell_types.npy', to_categorical(input_cell_types, num_cell_types))
    exit()
...
if task == 'integration_data_with_triple_modality':
    ...
    input_covariates1 = np.zeros(inputdata1.shape[0])
    input_covariates2 = np.ones(inputdata2.shape[0])
...
if task == 'cross_modal_generation':
    ...
    if inputdata1.shape[1] != inputdata3.shape[1]:
        ...
        exit()
    np.hstack((np.zeros(inputdata1.shape[0]), np.ones(inputdata2.shape[0])))
    ...
    input_covariates = to_categorical(input_covariates, 3)
