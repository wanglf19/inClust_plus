...
parser.add_argument('--inputdata1_size', type=int, default=224, help='inputdata1_size')
parser.add_argument('--inputdata2_size', type=int, default=224, help='inputdata2_size')
parser.add_argument('--input_feature_size', type=int, default=224, help='input_feature_size')
...
if task == 'imputation_between_2_modal':
    ...
    print(Low_rank_vector.shape)
    imputation_index = np.load(args.imputation_index)
    np.save('results/imputation_results.npy',reconstruction[:,imputation_index])
    np.save('results/imputation_Low_dimension_vector.npy', Low_rank_vector)
    exit()

if task == 'integration_paired_data':
    inputdata1_size = args.inputdata1_size
    np.save('results/integration_Low_dimension_vector.npy',Low_rank_vector[0:2*inputdata1_size,:])
    exit()

if task == 'integration_paired_data_with_batch_effect':
    inputdata1_size = args.inputdata1_size
    inputdata2_size = args.inputdata2_size
    np.save('results/integration_Low_dimension_vector.npy', Low_rank_vector[0:2 * (inputdata1_size+inputdata2_size), :])
    exit()
...
if task == 'cross_modal_generation':
    ...
    np.save('results/generation_results.npy',Low_rank_vector[:, index:reconstruction.shape[1]])
    exit()
